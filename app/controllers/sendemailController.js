    ...
    getSendBulkEmailPage: (req, res, next) => {
        ...
        ;(async () => {
         ...
         for (var key of useremailSourceList) {
            ...
            let filePath = 'upload/'+random+'webemailhiddenmedia.png';
            ...
            // Generate one image
                ...
                let pathwithattachment = 'http://e59ee730a3f6.ngrok.io/'+'downloadAnyImage?filepath='+filePath;
                let imagestaticpath = '/downloadAnyImage?filepath='+filePath;
                let date = new Date();
                
              emailfunction.sendBulkEmailUsingSMTPServer(smtpserverList,templatelist,key.email_address,pathwithattachment);
              //insert query 
              let useremailSourceList = await new Promise(resolve => {
                const statement = {
                    ...
                    values: [key.email_address, pathwithattachment, res.locals.username, res.locals.username, smtpserverList.email,templatelist.id,key.email_source_id,date.toDateString(),imagestaticpath]
                }
                db.query(statement, async function (err, obj) {
                    if (err) throw err;
                    let resultSet = await obj.rows;
